                                   ...
                                   const secp256k1_scalar* k)
{
    if (i_cycle != ag->i_cycle_trg) {
        ...
        if (ag->calc) {
            ...
            multi_mac_prepared_t gen_ipp;
            memcpy(&gen_ipp, get_generator_ipp(i_pos, ag->j, 0), sizeof(multi_mac_prepared_t));
            memcpy(&ag->mm->prepared[ag->mm->n_prepared++], &gen_ipp, sizeof(multi_mac_prepared_t));
        }
    }
}
                                         ...
                                         int j)
{
    ...
    multi_mac_prepared_t gen_dot_ipp;
    memcpy(&gen_dot_ipp, get_generator_dot_ipp(), sizeof(multi_mac_prepared_t));
    memcpy(&calc->mm.prepared[calc->mm.n_prepared++], &gen_dot_ipp, sizeof(multi_mac_prepared_t));
    ...
    if (ab) {
        for (uint32_t j = 0; j < 2; j++) {
            for (uint32_t i = 0; i < INNER_PRODUCT_N_DIM; i++, calc.mm.n_prepared++) {
                ...
                memcpy(&calc.mm.prepared[calc.mm.n_prepared], get_generator_ipp(i, j, 0), sizeof(multi_mac_prepared_t));
                calculator_modifier_expanded_set(
