                                  ...
                                  uint8_t* checksum)
{
    ...
    nonce_generator_export_output_key(&nonce, NULL, 0, NULL);
    memcpy(checksum, nonce.okm, DIGEST_LENGTH);
}

void rangeproof_public_get_msg(rangeproof_public_t* rp, uint8_t* hash32,
                               beam_sha256_ctx* oracle)
{
    beam_hash_sha256_write_64(oracle, rp->value);
    beam_hash_sha256_update(oracle, (const uint8_t* )&rp->recovery, sizeof(rp->recovery));
                              ...
                              beam_sha256_ctx* oracle)
{
    ...
    if (out->value >= _RANGEPROOF_AMOUNT_MINIMUM_VALUE) {
        ...
        memset(&out->recovery.checksum, 0, DIGEST_LENGTH);
        assign_aligned(out->recovery.kid.idx, (uint8_t* )&cp->kidv.id.idx,
                       sizeof(out->recovery.kid.idx));
        assign_aligned(out->recovery.kid.type, (uint8_t* )&cp->kidv.id.type,
                       sizeof(out->recovery.kid.type));
        assign_aligned(out->recovery.kid.sub_idx, (uint8_t* )&cp->kidv.id.sub_idx,
                       sizeof(out->recovery.kid.sub_idx));

        printf("RP pub create: xcrypt kid prepare\n");
        rangeproof_public_xcrypt_kid(&(out->recovery.kid), cp,
                                     out->recovery.checksum);
        printf("RP pub create: xcrypt kid done\n");

        uint8_t hash_value[DIGEST_LENGTH];
        printf("RP pub create: get msg\n");
        rangeproof_public_get_msg(out, hash_value, oracle);
        printf("RP pub create: signature sign prepare\n");
        signature_sign(hash_value, sk, get_context()->generator.G_pts,
                       &out->signature);
        printf("RP pub create: signature sign done\n");
    }
}
                                    ...
                                    uint8_t is_public)
{
    ...
    if (is_public) {
        ...
        beam_hash_sha256_write_64(&oracle, incubation);
                                    ...
                                    const secp256k1_gej* h_gen)
{
    ...
    printf("Got into rangeproof_confidential_create\n");

    beam_sha256_ctx copy_oracle;
    memcpy(&copy_oracle, oracle, sizeof(beam_sha256_ctx));
    uint8_t seed_sk[DIGEST_LENGTH];

#ifdef BEAM_DEBUG
    memset(seed_sk, 1, DIGEST_LENGTH);
#else
    memset(seed_sk, 1, 32);
    //random_buffer(seed_sk, sizeof(seed_sk));
#endif

    printf("About to sha256_oracle_update_sk\n");
    sha256_oracle_update_sk(&copy_oracle, sk);
    printf("About to beam_hash_sha256_update\n");
    beam_hash_sha256_update(&copy_oracle, seed_sk, sizeof(seed_sk));
    printf("About to beam_hash_sha256_write_64\n");
    beam_hash_sha256_write_64(&copy_oracle, cp->kidv.value);
    printf("About to sha256_oracle_create\n");
    sha256_oracle_create(&copy_oracle, seed_sk);

    printf("About to rangeproof_confidential_co_sign \n");
...
// NOT enough memory for NANOS. Just empty function there
int rangeproof_confidential_co_sign(rangeproof_confidential_t* out,
                                    ...
                                    const secp256k1_gej* h_gen)
{
    printf("I'm a empty rangeproof_confidential_co_sign function for Nano S!");
}
#else
int rangeproof_confidential_co_sign(rangeproof_confidential_t* out,
                                    const uint8_t* seed_sk,
                                    ...
                                    const secp256k1_gej* h_gen)
{
    ...
    static_assert(sizeof(rangeproof_creator_params_padded_t) == DIGEST_LENGTH);
    rangeproof_creator_params_padded_t pad;
    memset(pad.padding, 0, sizeof(pad.padding));
    assign_aligned(pad.v.id.idx, (uint8_t* )&cp->kidv.id.idx,
                   sizeof(pad.v.id.idx));
    assign_aligned(pad.v.id.type, (uint8_t* )&cp->kidv.id.type,
                   sizeof(pad.v.id.type));
    assign_aligned(pad.v.id.sub_idx, (uint8_t* )&cp->kidv.id.sub_idx,
                   sizeof(pad.v.id.sub_idx));
    assign_aligned(pad.v.value, (uint8_t* )&cp->kidv.value, sizeof(pad.v.value));
    ...
    secp256k1_gej comm;
    {
        ...
        for (int j = 0; j < 2; j++)
            for (uint32_t i = 0; i < MAX_DIM_IN_MEMORY; i++) {
                ...
                // Test to fill with generators
                //memcpy(&mm.prepared[mm.n_prepared++], get_generator_G(), sizeof(multi_mac_prepared_t));
                // Test to fill with generators (another syntax construct)
                //mm.prepared[mm.n_prepared++].pt[0] = *get_generator_G();
