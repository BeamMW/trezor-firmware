...
#else
    //TODO<Kirill A> add this in Ledger
    //random_buffer((uint8_t*)&kidv->id.idx, sizeof(kidv->id.idx));

    // Test version
    test_set_buffer((uint8_t*)&kidv->id.idx, sizeof(kidv->id.idx), 0);
    //beam_rng((uint8_t*)&kidv->idx.idx, sizeof(kidv->id.idx));
#endif
    kidv->id.sub_idx = 0;
...
void get_seed_kid_from_commitment(const point_t* commitment, uint8_t* seed,
    ...
    beam_hash_sha256_update(&hp, commitment->x, DIGEST_LENGTH);
    beam_hash_sha256_write_8(&hp, commitment->y);
    beam_hash_sha256_final(&hp, seed);
    ...
    beam_sha256_ctx hp2;
    beam_hash_sha256_init(&hp2);
    beam_hash_sha256_update(&hp2, sk_data, DIGEST_LENGTH);
    beam_hash_sha256_final(&hp2, seed);
}
